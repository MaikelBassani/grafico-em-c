...
int main (void){
	...
	for (qualSort; qualSort < 5; qualSort++)
	{
		if (qualSort == 0){
			...
			//w - Abre somente para gravação e coloca o ponteiro no começo do arquivo e apaga o conteúdo que já foi escrito.
			fprintf (pFile1, "[{\n");
		...
		//a+ - Abre o arquivo para leitura e gravação e coloca o ponteiro no fim do arquivo.
		...
		for (operacao = 1; operacao <= 3; operacao++){
			...
			switch (qualSort){
				case 0:
					organizadores = bubbleSort(geraNumero(vetor, operacao));
					break;
					
				case 1:
					organizadores = insertionSort(geraNumero(vetor, operacao));
					break;
					
				case 2:
					organizadores = selectionSort(geraNumero(vetor, operacao));
					break;
					
				case 3:
					organizadores = mergeSort(geraNumero(vetor, operacao),0,QTD - 1);
					break;
					
				case 4:
					organizadores = quickSort(geraNumero(vetor, operacao),0,QTD - 1);
					break;
					
			}
			fprintf (pFile1, "\t\"Titulo\" : \"%s - %s\",\n", organizadores.nome, teste);
			fprintf (pFile1, "\t\"Comparacoes\" : %d,\n",organizadores.comparacoes);
			fprintf (pFile1, "\t\"Trocas\" : %d,\n",organizadores.troca);
			fprintf (pFile1, "\t\"Segundos\" : %.2f\n",organizadores.segundos);
