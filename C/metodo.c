...
Sort bubbleSort(int vetor[]) {
	...
	int i = 0;
	int aux;
	bolha[operacao] .comparacoes= 0;
	bolha[operacao].troca = 0;
	bolha[operacao].segundos = 0;
	while (n <= QTD && troca == 1) {
		troca = 0;
		for (i = 0; i <= QTD - 2; i++) {
			bolha[operacao].comparacoes ++;
			if (vetor[i] > vetor[i + 1]) {
				troca = 1;
				bolha[operacao].troca++;
				aux = vetor[i];
				vetor[i] = vetor[i + 1];
				vetor[i + 1] = aux;
			}
	...
	t = clock() - t;
    bolha[operacao].segundos = ((float)t)/CLOCKS_PER_SEC;
	return bolha;
}
